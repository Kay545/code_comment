...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:  # img0s是原图
        t1 = time_sync()
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det) >= 0:
                ...
                # 在此之前的值，是在resize后的图上的坐标，现在要把它还原到resize之前的坐标上，scale_coords这个函数可以还原
                det[:, :4] = scale_coords(im.shape[2:], det[:, :4], im0.shape).round()  # img0是原图

                # Print results
                # 这个是在终端输出的结果，每张图片检测到的class还有每个class的个数
                for c in det[:, -1].unique():  # c 打印的是类别对应的数字
                    n = (det[:, -1] == c).sum()  # detections per class  n打印的是某个类别的数量
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string
                    # info_1 = f"{n} {names[int(c)]}{'s' * (n > 1)}, "
                    # info = f"{names[int(c)]}{'s' * (n > 1)},"
                    # print(info_1)
