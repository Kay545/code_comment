...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    # 这行代码的作用是生成一个与shapes数组行数相同、列数为1的随机数数组，这个数组中的每个元素都在0.9到1.1之间，用于后续的数学运算或者数据处理中。这个应该是数据增强的裁剪的比列吧
    wh = torch.tensor(np.concatenate([l[:, 3:5] * s for s, l in zip(shapes * scale, dataset.labels)])).float()  # wh
    # 这行代码的作用是将dataset.labels中的一些特定列的数据与通过缩放因子shapes * scale相乘后得到的结果连接成一个PyTorch张量wh。
