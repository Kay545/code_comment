...
def check_dataset(data, autodownload=True):
    ...
    # Download (optional)
    extract_dir = ''
    # data 变量被检查是否是 str 类型或者 pathlib.Path 类型的实例。
    # isinstance，后面的括号是可选择的类型，如果data为str或者是path都可以， 如果为zip，则解压
    ...
    if not path.is_absolute(): # 是否为绝对路径 pathlib的函数
        path = (ROOT / path).resolve()
    for k in 'train', 'val', 'test': # 从yaml文件中获取key
        if data.get(k):  # prepend path
            data[k] = str(path / data[k]) if isinstance(data[k], str) else [str(path / x) for x in data[k]]

    # Parse yaml
    assert 'nc' in data, "Dataset 'nc' key missing."  # 获取类别的种类数量，如果没有则报错
    if 'names' not in data:
        data['names'] = [f'class{i}' for i in range(data['nc'])]  # assign class names if missing，分配names，如果miss就用数字补充
    train, val, test, s = (data.get(x) for x in ('train', 'val', 'test', 'download'))
    if val: # 验证集是否存在，不存在就会下载吧 ，不检查训练集吗
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    nc = prediction.shape[2] - 5  # number of classes   prediction是网络模型的直接输出, 输出其shape是(1, 50000（以50000为例子）, nc+5), 1表示的是图片的个数，50000表示是网络预测的候选框的个数，nc + 5 表示xywh 含有目标的概率，每个nc的概率
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # xi是图片的index，其值是0，表示第0张图片，x是图片的推理结果，其shape是torch.Size([50000, 7])，这里巧妙的利用了enumerate将图片的index和图片的推理结果分离开，分别存放在了xi和x里面。本来prediction是一个shape为torch.Size([1, 50000, 7])的三维张量，张量的轴1是图片的index，张量的另外两个轴对应图片的推理结果，这里利用了enumerate将prediction的这两部分给分离开了。这行代码很巧妙
        
        x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence
        
        """
        xc是之前计算出的shape为(1, 50000)的tensor, 每个值是True或者False, 【大于置信度的阈值为True】, 用其值表达每个box的置信度是否大于或者小于conf_thres值。xc[xi], [xi]这个是对xc tensor的取值方式, xi的值是0, 则表示取第0张图片的所有True或者False值。xc[xi]的shape是torch.Size([50000]), 与x的第0个维度一致, x[xc[xi]]也是对x tensor的取值方式, x[xc[xi]]整个表达式则表示对x第0个维度上xc[xi]所有为True的保留, 为False的则舍弃, 也就是对第i(这里i是0)张图片所有confidence值大于conf_thres的box取出, 然后重新赋值给x。
        """
